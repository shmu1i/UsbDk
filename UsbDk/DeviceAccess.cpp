...
bool CWdmUSBD::Create()
{
    ...
    {
        TraceEvents(TRACE_LEVEL_ERROR, TRACE_USBDK_DEVACCESS, "%!FUNC! IoCreateDevice failed, %!STATUS!", status);
        return false;
    }
    ...
    {
        TraceEvents(TRACE_LEVEL_ERROR, TRACE_USBDK_DEVACCESS, "%!FUNC! IoAttachDeviceToDeviceStack returned NULL");
        return false;
    }
    ...
    if (!NT_SUCCESS(status))
    {
        TraceEvents(TRACE_LEVEL_ERROR, TRACE_USBDK_DEVACCESS, "%!FUNC! USBD_CreateHandle failed, %!STATUS!", status);
        return false;
    }
...
CRegText* CWdmDeviceAccess::GetAddressString()
{
    ...
    {
        TraceEvents(TRACE_LEVEL_ERROR, TRACE_USBDK_DEVACCESS, "%!FUNC! Cannot convert port number to UNICODE_STRING.");
        return nullptr;
    }
    ...
    if (PortString->empty())
    {
        TraceEvents(TRACE_LEVEL_ERROR, TRACE_USBDK_DEVACCESS, "%!FUNC! Cannot allocate port string.");
        return nullptr;
    }
...
PWCHAR CWdmDeviceAccess::QueryBusID(BUS_QUERY_ID_TYPE idType)
{
    ...
    if (!NT_SUCCESS(status))
    {
        TraceEvents(TRACE_LEVEL_ERROR, TRACE_USBDK_DEVACCESS, "%!FUNC! Error %!STATUS! during IRP creation", status);
        return nullptr;
    }
    ...
    if (!NT_SUCCESS(status))
    {
        TraceEvents(TRACE_LEVEL_ERROR, TRACE_USBDK_DEVACCESS, "%!FUNC! Error %!STATUS! during %!devid! query", status, idType);
        return nullptr;
    }
...
NTSTATUS CWdmDeviceAccess::QueryCapabilities(DEVICE_CAPABILITIES &Capabilities)
{
    ...
    if (!NT_SUCCESS(status))
    {
        TraceEvents(TRACE_LEVEL_ERROR, TRACE_USBDK_DEVACCESS, "%!FUNC! Error %!STATUS! during capabilities query", status);
    }
...
bool CWdmDeviceAccess::QueryPowerData(CM_POWER_DATA& powerData)
{
    ...
    if (!NT_SUCCESS(status))
    {
        TraceEvents(TRACE_LEVEL_ERROR, TRACE_USBDK_DEVACCESS, "%!FUNC! Error %!STATUS!", status);
    }
...
)
{
    ...
    TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_USBDK_DEVACCESS, "%!FUNC! -> D%d", PowerState.DeviceState - 1);
    pev->Set();
}
...
PWCHAR CWdmDeviceAccess::MakeNonPagedDuplicate(BUS_QUERY_ID_TYPE idType, PWCHAR idData)
{
    ...
    {
        TraceEvents(TRACE_LEVEL_ERROR, TRACE_USBDK_DEVACCESS, "%!FUNC! Failed to allocate non-paged buffer for %!devid!", idType);
    }
    ...
    USHORT intfSize, USHORT intfVer, __in_opt PVOID intfCtx)
{
    ...
    if (!NT_SUCCESS(status))
    {
        TraceEvents(TRACE_LEVEL_ERROR, TRACE_USBDK_DEVACCESS, "%!FUNC! IRP alloc failed");
        return status;
    }
...
NTSTATUS CWdmUsbDeviceAccess::Reset(bool ForceD0)
{
    ...
    {
        TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_USBDK_DEVACCESS, "%!FUNC! device power state D%d", powerData.PD_MostRecentPowerState - 1);
    ...
    if (!NT_SUCCESS(status))
    {
        TraceEvents(TRACE_LEVEL_ERROR, TRACE_USBDK_DEVACCESS, "%!FUNC! Error %!STATUS! during IOCTL IRP creation", status);
        return status;
    }
    ...
    if (!NT_SUCCESS(status))
    {
        TraceEvents(TRACE_LEVEL_ERROR, TRACE_USBDK_DEVACCESS, "%!FUNC! Send IOCTL IRP Error %!STATUS!", status);
    }
...
NTSTATUS CWdmUsbDeviceAccess::GetConfigurationDescriptor(UCHAR Index, USB_CONFIGURATION_DESCRIPTOR &Descriptor, size_t Length)
{
    ...
    {
        TraceEvents(TRACE_LEVEL_ERROR, TRACE_USBDK_DEVACCESS, "%!FUNC! Invalid configuration descriptor on unknown size received.");
        return USBD_STATUS_INAVLID_CONFIGURATION_DESCRIPTOR;
    }

    if ((Descriptor.wTotalLength <= Length) && !NT_SUCCESS(status))
    {
        TraceEvents(TRACE_LEVEL_ERROR, TRACE_USBDK_DEVACCESS, "%!FUNC! Failed to retrieve the configuration descriptor.");
        return status;
    }
                               ...
                               CObjHolder<CRegText> *InstanceID)
{
    ...
    if (DeviceID != nullptr)
    {
        *DeviceID = pdoAccess.GetDeviceID();
        ...
        {
            TraceEvents(TRACE_LEVEL_ERROR, TRACE_USBDK_DEVACCESS, "%!FUNC! No Device IDs read");
            return false;
        }
    }
    ...
    if (InstanceID != nullptr)
    {
        *InstanceID = pdoAccess.GetInstanceID();
        ...
        {
            TraceEvents(TRACE_LEVEL_ERROR, TRACE_USBDK_DEVACCESS, "%!FUNC! No Instance ID read");
            return false;
        }
    }
...
bool UsbDkGetWdmDeviceDriverKeyName(const PDEVICE_OBJECT PDO, CObjHolder<CRegText>* DriverKeyName)
{
    ...
    {
        TraceEvents(TRACE_LEVEL_ERROR, TRACE_USBDK_DEVACCESS, "%!FUNC! Invalid paramteters.");
        return false;
    }
    ...
    if (!NT_SUCCESS(status))
    {
        TraceEvents(TRACE_LEVEL_ERROR, TRACE_USBDK_DEVACCESS, "%!FUNC! Can't get DevicePropertyDriverKeyName for PDO: %p, status: %!STATUS!", PDO, status);
        return false;
    }
    ...
    if (string->empty())
    {
        TraceEvents(TRACE_LEVEL_ERROR, TRACE_USBDK_DEVACCESS, "%!FUNC! Can't allocate memory");
        return false;
    }
...
NTSTATUS UsbDkSendUrbSynchronously(PDEVICE_OBJECT Target, URB &Urb)
{
    ...
    if (!NT_SUCCESS(status))
    {
        TraceEvents(TRACE_LEVEL_ERROR, TRACE_USBDK_DEVACCESS, "%!FUNC! Send URB IRP Error %!STATUS!", status);
    }
